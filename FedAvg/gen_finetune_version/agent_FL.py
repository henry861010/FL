...
class Agent_FL(Clients, Model, Recorder, Selector):
    ...
    def train(self):
        ...
        #for self.experiment_round_now in range(len(self.record), self.experiment_rounds_num):
        for self.experiment_round_now in range(self.experiment_rounds_num):
            print("---------------- start the experiment ",self.experiment_round_now," -----------------")

            # check if need to load the pretraind
            if self.experiment_round_now < len(self.record):
                pretrained_round = len(self.record[self.experiment_round_now]['accuracy'])
                print("pretrained_round: ",pretrained_round)

                if pretrained_round < self.global_rounds_num:
            ...
            # begin the training
            for round_num in range(pretrained_round, self.global_rounds_num):
                ...
                print(f"{self.experiment_id}-exp{self.experiment_round_now} round-{round_num}:")
                ...
                # evaluation
                self.evaluation(result, round_num, self.experiment_round_now, summary_writer, selected_ids)
                
                print("")
            #self.save_model(self.training_process, train_state, self.experiment_round_now)
